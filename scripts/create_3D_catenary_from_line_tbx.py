            ...
            if desc.shapeType in ["Polyline", "Line", "Point"]:
                ...
                if zValues:
                    if desc.shapeType == "Point":
                        ...
                        if common_lib.check_fields(input_features, [line_attribute, tower_attribute], False,
                                                   verbose) == 0:
                            # process points
                            ...
                            if in_memory_switch:
                                ...
                                pointToLines = os.path.join(scratch_ws, lines_name)

                            arcpy.PointsToLine_management(input_features, pointToLines, line_attribute,
                                                          tower_attribute)

                            input_features = pointToLines
                        else:
                            arcpy.AddError(
                                "Input feature class requires a '" + line_attribute + "' and '" + tower_attribute + "' attribute. Exiting...")
                            input_features = None

                        if len(tower_height) > 0:
                            if common_lib.check_fields(input_features, [tower_height], False,
                                                   verbose) == 0:
                            ...
                            if haveTables:
                                ...
                                if catenary and TowerModels and JunctionPoints and TowerPlacementPoints:
                                    if arcpy.Exists(catenary) and arcpy.Exists(TowerModels) and\
                                        ...
                                        arcpy.SetParameter(9, TowerPlacementPoints)
                                        arcpy.SetParameter(10, JunctionPoints)
                                        arcpy.SetParameter(11, TowerModels) # reordered these lines just for good chi.
                                        ...
                                        if output_layer4:
                                            arcpy.SetParameter(12, output_layer4)
                                        else:
                ...
                end_time = time.clock()
                msg_body = create_msg_body("Only input feature type: Point, PolyLine supported currently. Exiting...", start_time, end_time)
                msg(msg_body, WARNING)
