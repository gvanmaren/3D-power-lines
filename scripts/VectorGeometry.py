...
Last updates from cw on 8-24: Kept all of gerts changes, and then cleaned up some of my old code.
"""
...
def funPointToArcpyPoint(funPoint):
    if funPoint is None:
        pint("Error: funPointToArcpyPoint: input is None")
        return None
    else:
...
def arcpyPolygonToFunPolygon(arcpyPolygon):
    ...
    if pointsAreCoincident(firstPoint, lastPoint, 0.0000000001):
        #pint("Warning: Polygon has repeated end node. Node is ignored for tool output, but original feature still has it.")
...
class SetbackPerEdge(object):
    def __init__(self, floorPlate, ringIndex, subPolyIndex):
        pint("New SetbackPerEdge, ring# " + str(ringIndex))
    ...
    def make(self):
        ...
        pint("Find components for skeleton.")
        ...
        for index in range(0,len(self.vectorsA)):
            ...
            if turnAngle == 0:
                ...
                p("angleAlpha",angleAlpha)
                cosineForBothVectors = math.cos(angleAlpha)
                p("cosineForBothVectors",cosineForBothVectors)
        ...
        pint("Handle Setback Ring Events")
        #p("pokeEventAtLowest",pokeEventAtLowest)
        #p("distancePerSetbackLowest",distancePerSetbackLowest)
        ...
        if lowestEventPerSetback > 1:
            ...
            pint("Edge Collapse on Ring # " + str(self.ringIndex))
            ...
            # INTERMEDIATE EVENT POLYGON SPLIT
                ...
                pint("Poke event on Ring # " + str(self.ringIndex))
                ...
                pint("Warning: WEIRD: unhandled SetbackPerEdge event.")
        pint("End SetbackPerEdge")
        pass



def SetbackPerEdgeToArea(floorPlate, targetArea, stepbackVariabilityList):
    pint("Start SetbackPerEdgeToArea search function")
    try:
        completed = False
        maxLoopCount = 30
        ...
        #    pint("Error: SetbackPerEdge cannot grow in area. It can only decrease.")
        #    raise Exception
        if len(floorPlate.edgeDataList) != len(stepbackVariabilityList):
            pint("Error: SetbackPerEdgeToArea has mismatched variability filter for scaling.")
            raise Exception
            ...
            while not completed and index < maxLoopCount:
                ...
                if setbackPerEdge is None: # it either had an unhandled exception, or was too deep a stepback.
                    ...
                    if index > maxLoopCount:
                        completed = True
                        pint("Max loop count hit in SetbackPerEdge Area.")

            pint("End getSetbackPerEdgeByArea search function")
            return setbackPerEdge
            # End While.

    except Exception:
        pint("Failure in SetbackPerEdgeToArea")
        e = sys.exc_info()[1]
        arcpy.AddMessage(e.args[0])
